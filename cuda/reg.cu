...
void check_cuda(std::string& error_msg)
{
    ...
    {
        error_msg = "Failed to obtain NVIDIA GPU driver and device information (CUDA ERROR ";
        error_msg += std::to_string(int(cudaGetDeviceCount(&gpu_count)));
        error_msg += "). Please install/update the NVIDIA driver or use the CPU version.";
        return;
    }
